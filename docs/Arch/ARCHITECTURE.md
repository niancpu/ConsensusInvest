...
| [runtime_persistence.md](./runtime_persistence.md) | 运行态持久化、状态归属、恢复语义和队列边界。 |
...
- `AgentRuntime`：通用运行层，只管理 Agent 类任务的生命周期、状态、事件、预算、错误和 trace。它写入统一事件日志 `runtime_events`，但不是通用业务 Agent 基类，不定义 Search、Debate、Judge 的输入输出语义。
...
运行态持久化是横切边界：所有已经对外返回 ID 的任务、workflow、agent run 和 report run，都必须先有持久状态，再执行异步工作。事件流和内存对象不能作为运行态事实来源。
...
- MVP 使用 SQLite 作为运行态事实来源；独立任务队列后续只能作为调度层，不能替代 `search_tasks`、`workflow_runs`、`agent_runs`、`report_runs` 等运行表。
- Report Module 的 `report_runs` 第一版保存完整输入输出快照，用于复现用户当时看到的报告视图。
- AgentRuntime 第一版写统一事件日志表 `runtime_events`，用于 Agent 类任务 trace / audit。
