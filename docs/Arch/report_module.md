...
`report_run_id` 对应的运行状态必须写入 `report_runs`。报告视图可以使用缓存，但缓存不能替代 `report_runs` 的状态、输入引用和限制说明。

`report_runs` 第一版保存完整输入输出快照：

- 输入快照：报告对象、生成参数、Evidence / MarketSnapshot / Judgment / Workflow 引用、数据状态和限制条件；
- 输出快照：用户当时看到的报告视图结构、章节、引用、限制说明和生成时间；
- 错误快照：失败阶段、可展示错误和可排查错误。

保存完整快照的目的，是复现用户当时看到的报告视图。快照不代表 Evidence Store 的当前最新事实，也不能把报告视图提升为 Judgment。
