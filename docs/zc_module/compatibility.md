...
| 旧接口 | 新协议处理 |
| --- | --- |
| `GET /api/v1/stocks/search` | 保留为 ZC 模块正式视图接口；结果来自 Entity / Evidence Store / 已有股票索引。 |
...
| `GET /api/v1/stocks/{stockCode}/event-impact-ranking` | 保留为事件影响排序视图；从 Evidence / 主链路 Trace / ZC 模块内结果派生。 |
| `GET /api/v1/stocks/{stockCode}/benefits-risks` | 保留为利好风险视图；字符串数组升级为带引用 ID 的对象数组。 |
...
- 数据不足时，只能请求主系统已有 Search Agent / 数据刷新能力异步补齐；ZC 模块接口不能阻塞等待完整搜索完成。
- Search Agent 或行情刷新拿到的新事实必须进入 Raw Item / Evidence / Market Snapshot 等主系统数据层，ZC 模块再读取结果。
- ZC 模块内 Agent 只负责模块内解释、排序、归纳，不替代主链路 Perspective Agents / Judge Agent。
- ZC 模块返回的解释性字段必须带 `evidence_ids`、`entity_ids`、`workflow_run_id`、`judgment_id` 或 `zc_run_id` 中至少一种可追踪引用。

前端注解：

- 新页面不要调用旧 `analysis/*` 和 `reports/*`。
- 旧页面如果还有这些调用，应直接改到 `docs/web_api` 的 `workflow-runs` 或本目录的 `stocks/*` 聚合视图。
- 如果 UI 只展示 ZC 聚合结果，也要保留“查看依据”入口，否则会破坏金融 Agent 工具的透明性。
